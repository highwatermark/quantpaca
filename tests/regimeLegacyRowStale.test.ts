...
const dataDir = fs.mkdtempSync(path.join(os.tmpdir(), "quantpaca-regime-legacy-row-"));
...
test("migration safety: a persisted row with no fetchedAt field at all is treated stale and refetches", async (t) => {
  const listener = app.listen(0);
  const port = (listener.address() as { port: number }).port;
  t.after(() => listener.close());

  // Seed a row shaped like it was persisted before `fetchedAt` existed: a
  // recent `asOf` (which, under the OLD inverted semantics, would have looked
  // "fresh enough" to reuse) but no `fetchedAt` field at all.
  ...
  rawStore.saveRegimeAssessment({
    ...
    sizeMultiplier: 0.3,
    reason: "legacy pre-fetchedAt fixture row",
    inputs: { spyTrendPercent: -5 },
    // fetchedAt intentionally omitted -- this is the point of the test.
  } as any);
  rawStore.close();
  ...
  assert.equal(barsRequestCount, 3, "a legacy row with no fetchedAt must be treated stale and refetch (3 requests)");

  const store = createProductionStore(sqlitePath);
  const regime = store.latestRegimeAssessment();
  store.close();
  assert.notEqual(regime!.id, "reg-legacy", "the legacy seed row must not simply be reused as-is");
  assert.equal(regime!.marketMode, "risk_on");
